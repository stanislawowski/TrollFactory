...
def generate():
    ...
    print('\n')
    output('Generating identity [{}, {}, {}]..'.format('no preset', language.capitalize(), sex))
    ...
    while (len(properties) > 0):
        for prop_name in properties:
            ...
            output('Prop name: {}'.format(prop_name))

            prop_attrs = prop_class.generate(properties_static)

            for key, value in sorted(prop_attrs.items()):
                output('    {}: {}'.format(key, value))

            properties_static[prop_name] = prop_attrs
            properties.remove(prop_name)

def generate_web(language, sex, output_type):
    output_lines = []
    properties = load_props()
    if language == 'polish': country_code = 'PL'
    properties_static = {'language': {'language': language, 'country_code': country_code}, 'sex': {'sex': sex}}
    ...
    while len(properties) > 0:
        for prop_name in properties:
            ...
            output_lines.append('Prop name: {}'.format(prop_name))
            prop_attrs = prop_class.generate(properties_static)
            for key, value in sorted(prop_attrs.items()): output_lines.append('    {}: {}'.format(key, value))
            properties_static[prop_name] = prop_attrs
            properties.remove(prop_name)
            output_lines.append('End prop')
    if output_type == 'normal': return(output_lines)
    elif output_type == 'json': return(dumps(properties_static))
